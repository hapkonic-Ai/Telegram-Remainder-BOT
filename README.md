...
- **Serverless Architecture**: Runs entirely on Vercel API routes using Webhooks and Cron Jobs, meaning zero hosting costs for typical usage.
...
- `api/cron.js` - A Vercel Cron scheduled endpoint that runs every minute to check the database for any due reminders and sends them out.
